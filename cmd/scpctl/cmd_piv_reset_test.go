...
// TestPIVGroupReset_HappyPath_BlocksThenResets is the regression
// test for the bug rmhrisk hit on retail YubiKey 5.7.4: 'scpctl
// piv reset' (the group command) was sending RESET without first
// blocking PIN and PUK, which on real hardware returned SW=6985.
// 'scpctl smoke piv-reset' worked because it blocks first; the
// group command went through piv/session.Session.Reset which
// (intentionally) leaves blocking to the caller.
//
// Fix: cmdPIVGroupReset now calls sess.BlockPIN + sess.BlockPUK
// before sess.Reset, and emits the same PASS-step structure as
// the smoke command so an operator looking at one report can map
// step-for-step onto the other.
//
// The test pins the new step structure exactly. If a future
// change drops one of the block steps or stops emitting the
// PASS line for it, this test fails loudly with a clear pointer
// to the regression class.
//
// Uses --raw-local-ok with a TrustBoundary-overriding wrapper
// so the mock transport satisfies the production raw-mode gate
// without spinning up a full SCP11b vs PIV session against the
// mock (which would require extensive SCP key fixture wiring
// outside the scope of this regression).
