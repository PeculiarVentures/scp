...
- **Additional non-YubiKey GP cards** as fully verified profiles. Hardware exposure for the unauthenticated read paths (SD discovery, CRD, CPLC, IIN, CIN, KDD, SSC, Card Capabilities, GET KEY INFORMATION) is in place across SafeNet eToken Fusion, SafeNet Token JC, ML840, Oberthur, GoldKey, Feitian, and Treasury Gemalto; the parser handles spec-conformance variation (FCI-wrapped vs bare CRD, 61xx chaining, 6Cxx Le-retry, vendor-specific KIT shapes). Authenticated write paths (PUT KEY, STORE DATA, DELETE KEY) and the corresponding profile promotion remain pending hardware exposure to a non-YubiKey card whose factory keys are available for the test session.
...
- **SCP11 HostID / CardGroupID wire behavior**. The host-identifier path is intentionally absent from `scp11.Config` today: AUTHENTICATE parameter bit, tag-`0x84` TLV in the control reference template, and matching KDF shared-info would all need to be implemented together against transcript vectors from a card or reference implementation that exercises the path. Until those vectors are available — committing to a wire-side encoding without ground truth would silently establish sessions under a different key schedule than the card — the fields are deliberately not part of the public API. Adding them is in scope when transcripts surface.
